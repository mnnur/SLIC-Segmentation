...
def run_normalized_cuts(image_np, n_segments, compactness):
    ...
    labels = slic(image_gaussian, compactness=compactness, n_segments=n_segments, start_label=1, convert2lab=True)
        ...
        try:
            if method == "SLIC":
                with st.spinner("Running SLIC segmentation..."):
                    colorized, boundaries, label_map = run_slic(image_gaussian, int(n_segments), float(compactness))
            else:
            ...
            st.error("Segmentation failed due to an ARPACK error (e.g., not enough shifts during eigenvalue computation). "
                     "Try increasing the number of segments or reduce compactness.")
            st.exception(e)
        except Exception as e:
            st.error("An unexpected error occurred during segmentation.")
            st.exception(e)
